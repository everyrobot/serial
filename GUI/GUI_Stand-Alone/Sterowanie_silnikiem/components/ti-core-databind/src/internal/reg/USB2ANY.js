...
(function()
{
    ...
    I2C_Interface.prototype.read = function(u2a, info, registerModel, coreIndex)
    {
        ...
        if (this.crcUser && this.crc) {
            ...
            return u2a.readResponse(u2a.sendCommandPacket(cmd, this.readData)).then(getResult);
        }
    };
    ...
    I2C_Interface.prototype.multiRegisterRead = function(u2a, startRegisterInfo, count, registerModel, coreIndex)
    {
        ...
        if (count > MAX_COUNT)
        {
            ...
            while(count > MAX_COUNT)
            {
                ...
                promises.push(u2a.readResponse(u2a.sendCommandPacket(Command.Cmd_I2C_ReadWithAddress, this.readData)).then(getMultipleRegisterReadResult));
                count -= MAX_COUNT;
        ...
        else
        {
            ...
            this.readData[3] = count;
            return u2a.readResponse(u2a.sendCommandPacket(Command.Cmd_I2C_ReadWithAddress, this.readData)).then(getMultipleRegisterReadResult);
        }
    };
    ...
    I2C_Interface.prototype.write = function(u2a, info, value, registerModel, coreIndex)
    {
        ...
        setBytes(this.writeData, nBytes, value, 4);
    ...
    USB2ANY.prototype.checkFirmware = function(info) { // Ask Paul - put this as a base method in AbstractMessageBasedCode, IPollingPacketCode, or IPacketCode?
        var promises = [];
        for (x in firmwareChecks) {
            promises.push(firmwareChecks[x](info));
        }
    ...
    USB2ANY.prototype.connect = function(settings)
    {
        ...
        return this.u2aOpen().timeout(250, "No response from USB2ANY controller.").then(function()
        {
            ...
            return that.checkFirmware({detectedFirmwareVersion: that.version, modelID: settings.length && settings[0]._registerModel._id, codec: that, controller: 'usb2any'});
        }).then(function()
        {
            var timeoutInMs = 250;
